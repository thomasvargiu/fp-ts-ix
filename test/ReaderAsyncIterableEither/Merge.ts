...
import * as _ from '../../src/ReaderAsyncIterableEither/Merge'
...
describe('ReaderAsyncIterableEither#Merge', () => {
  ...
  it.concurrent('getSemigroup', async () => {
    ...
    await pipe(S_.concat(fromArray([1, 2]), fromArray([3, 4])), run({}), deepStrictEqual([1, 3, 2, 4].map(E.right)))
  })
  ...
  describe('getMonoid', () => {
    ...
    it.concurrent('concat', async () => {
      await pipe(M_.concat(fromArray([1, 2]), fromArray([3, 4])), run({}), deepStrictEqual([1, 3, 2, 4].map(E.right)))
    })
  ...
  it.concurrent('flatten', async () => {
    await pipe(
      ...
      _.flatten,
      run({}),
      deepStrictEqual([1, 3, 2, 4].map(E.right))
    )
  })
