...
import * as _ from '../../src/AsyncIterableEither/Merge'
...
describe('AsyncIterableEither#Merge', () => {
  ...
  it.concurrent('getSemigroup', async () => {
    ...
    await pipe(S_.concat(fromArray([1, 2]), fromArray([3, 4])), deepStrictEqual([1, 3, 2, 4].map(E.right)))
  })
  ...
  describe('getMonoid', () => {
    ...
    it.concurrent('concat', async () => {
      await pipe(M_.concat(fromArray([1, 2]), fromArray([3, 4])), deepStrictEqual([1, 3, 2, 4].map(E.right)))
    })
  ...
  it.concurrent('chain', async () => {
    ...
    await pipe(
      ...
      _.chain((a) => fromArray([a, a * 3])),
      deepStrictEqual([E.right(1), E.left('err'), E.right(3)])
    )
  })
  ...
  it.concurrent('flatten', async () => {
    await pipe(fromArray([fromArray([1, 2]), fromArray([3, 4])]), _.flatten, deepStrictEqual([1, 3, 2, 4].map(E.right)))
  })
  ...
  it.concurrent('chainFirst', async () => {
    await pipe(
      ...
      deepStrictEqual([1, 2, 1, 2].map(E.right))
    )
  })
