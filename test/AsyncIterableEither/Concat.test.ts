...
import * as _ from '../../src/AsyncIterableEither/Concat'
...
describe('AsyncIterableEither#Concat', () => {
  ...
  it.concurrent('chain', async () => {
    ...
    await pipe(
      ...
      _.chain((a) => fromArray([a, a + 1])),
      deepStrictEqual([E.right(1), E.right(2), E.left('err')])
    )
  })
  ...
  it.concurrent('flatten', async () => {
    await pipe(fromArray([fromArray([1, 2]), fromArray([3, 4])]), _.flatten, deepStrictEqual([1, 2, 3, 4].map(E.right)))
  })
  ...
  it.concurrent('chainFirst', async () => {
    await pipe(
      ...
      deepStrictEqual([1, 1, 2, 2].map(E.right))
    )
  })
