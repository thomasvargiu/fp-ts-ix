...
import * as _ from '../../src/ReaderAsyncIterable/Merge'
...
describe('ReaderAsyncIterable#Merge', () => {
  ...
  it.concurrent('getSemigroup', async () => {
    ...
    await pipe(S_.concat(fromArray([1, 2]), fromArray([3, 4])), run({}), deepStrictEqual([1, 3, 2, 4]))
  })
  ...
  describe('getMonoid', () => {
    ...
    it.concurrent('concat', async () => {
      await pipe(M_.concat(fromArray([1, 2]), fromArray([3, 4])), run({}), deepStrictEqual([1, 3, 2, 4]))
    })
  ...
  it.concurrent('flatten', async () => {
    await pipe(fromArray([fromArray([1, 2]), fromArray([3, 4])]), _.flatten, run({}), deepStrictEqual([1, 3, 2, 4]))
  })
