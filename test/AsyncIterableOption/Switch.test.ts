...
import * as _ from '../../src/AsyncIterableOption/Switch'
...
describe('AsyncIterableOption#Switch', () => {
  describe('ChainWithIndex', () => {
    ...
    it.concurrent('chain', async () => {
      await pipe(
        ...
        deepStrictEqual([2, 3].map(O.some))
      )
    })
  ...
  it.concurrent('flatten', async () => {
    await pipe(fromArray([fromArray([1, 2]), fromArray([3, 4])]), _.flatten, deepStrictEqual([3, 4].map(O.some)))
  })
  ...
  it.concurrent('chainFirst', async () => {
    await pipe(
      ...
      deepStrictEqual([2, 2].map(O.some))
    )
  })
