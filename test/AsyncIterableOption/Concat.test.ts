...
import * as _ from '../../src/AsyncIterableOption/Concat'
...
describe('AsyncIterableOption#Concat', () => {
  ...
  it.concurrent('getSemigroup', async () => {
    ...
    await pipe(S_.concat(fromArray([1, 2]), fromArray([3, 4])), deepStrictEqual([1, 2, 3, 4].map(O.some)))
  })
  ...
  describe('getMonoid', () => {
    ...
    it.concurrent('concat', async () => {
      await pipe(M_.concat(fromArray([1, 2]), fromArray([3, 4])), deepStrictEqual([1, 2, 3, 4].map(O.some)))
    })
  ...
  it.concurrent('flatten', async () => {
    await pipe(fromArray([fromArray([1, 2]), fromArray([3, 4])]), _.flatten, deepStrictEqual([1, 2, 3, 4].map(O.some)))
  })
  ...
  it.concurrent('chainFirst', async () => {
    await pipe(
      ...
      deepStrictEqual([1, 1, 2, 2].map(O.some))
    )
  })
