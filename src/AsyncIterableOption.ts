 ...
 * @since 0.1.0
 */
export const fromTaskEither: <E = never, A = never>(me: TE.TaskEither<E, A>) => AsyncIterableOption<A> = flow(
  TE.match(() => O.none, O.some),
  AI.fromTask
)
 ...
 * @since 0.1.0
 */
export const tryCatch = <A>(f: Lazy<AsyncIterable<A>>): AsyncIterableOption<A> =>
  pipe(
    IX.defer(() => IX.from(f())),
    AI.map(O.some),
...
export const tryCatchK =
  <A extends ReadonlyArray<unknown>, B>(f: (...a: A) => AsyncIterable<B>): ((...a: A) => AsyncIterableOption<B>) =>
  (...a) =>
    tryCatch(() => f(...a))
...
const _chain =
  (AIC: AIChain): Chain1<URI>['chain'] =>
  (fa, f) =>
    pipe(fa, chain(AIC)(f))
const _chainWithIndex =
  (AIC: AIChainWithIndex): ChainWithIndex1<URI, number>['chainWithIndex'] =>
  (fa, f) =>
