...
import type { Alt2, Alt2C } from 'fp-ts/Alt'
...
import type { RefinementWithIndex, PredicateWithIndex } from 'fp-ts/FilterableWithIndex'
import {
  FromEither2,
 ...
 * import { tryCatch } from 'fp-ts/AsyncIterableEither'
 *
...
const _alt =
  (AIC: AIChain): Alt2<URI>['alt'] =>
  (fa, that) =>
    pipe(fa, alt(AIC)(that))
...
export function getApplicativeAsyncIterableValidation<E>(A: Apply1<AI.URI>, S: Semigroup<E>): Applicative2C<URI, E> {
  const aap = ap_(A, E.getApplicativeValidation(S))
  return {
    URI,
    _E: undefined as any,
    map: _map,
    ap: (fab, fa) => pipe(fab, aap(fa)),
    of
  }
}
 ...
 * @since 2.10.0
 */
export const getAlt: (C: AIChain) => Alt2<URI> = (C) => ({
  URI,
  map: _map,
  alt: _alt(C)
})
...
export const filterMapWithIndex =
  ...
  <E>(fa: AsyncIterableEither<E, A>): AsyncIterableEither<E, B> =>
    AI.FilterableWithIndex.filterMapWithIndex(
      fa,
      (i, ea): O.Option<E.Either<E, B>> => (E.isLeft(ea) ? O.some(ea) : pipe(f(i, ea.right), O.map(E.right)))
    )
//chainAsyncIterableK(flow(AI.of, AI.filterMapWithIndex(f)))
...
export const partitionMap: <A, B, C>(
  f: (a: A) => E.Either<B, C>
) => <E>(fa: AsyncIterableEither<E, A>) => Separated<AsyncIterableEither<E, B>, AsyncIterableEither<E, C>> = (f) =>
...
export const partitionMapWithIndex =
  ...
  <E>(fa: AsyncIterableEither<E, A>): Separated<AsyncIterableEither<E, B>, AsyncIterableEither<E, C>> =>
    separated(
...
export const separate: <E, A, B>(
  fa: AsyncIterableEither<E, E.Either<A, B>>
) => Separated<AsyncIterableEither<E, A>, AsyncIterableEither<E, B>> =
  /*#__PURE__*/
 ...
 */
export const filter: {
  <A, B extends A>(refinement: Refinement<A, B>): <E>(as: AsyncIterableEither<E, A>) => AsyncIterableEither<E, B>
  <A>(predicate: Predicate<A>): <E, B extends A>(bs: AsyncIterableEither<E, B>) => AsyncIterableEither<E, B>
  <A>(predicate: Predicate<A>): <E>(as: AsyncIterableEither<E, A>) => AsyncIterableEither<E, A>
 ...
 *
 * @example
 * import { filterWithIndex } from 'fp-ts-iterable/AsyncIterable';
 *
...
export const filterWithIndex: {
  <A, B extends A>(refinementWithIndex: RefinementWithIndex<number, A, B>): <E>(
    as: AsyncIterableEither<E, A>
  ) => AsyncIterableEither<E, B>
  <A>(predicateWithIndex: PredicateWithIndex<number, A>): <E, B extends A>(
    bs: AsyncIterableEither<E, B>
  ) => AsyncIterableEither<E, B>
  <A>(predicateWithIndex: PredicateWithIndex<number, A>): <E>(
    as: AsyncIterableEither<E, A>
  ) => AsyncIterableEither<E, A>
} =
  <A>(predicateWithIndex: PredicateWithIndex<number, A>) =>
  <E>(as: AsyncIterableEither<E, A>): AsyncIterableEither<E, A> =>
    AI.FilterableWithIndex.filterWithIndex(as, (i, a) =>
      pipe(
        a,
        E.match(
          () => true,
          (b) => predicateWithIndex(i, b)
        )
      )
    )
...
export const partition: {
  ...
  <A>(predicate: Predicate<A>): <E, B extends A>(
    bs: AsyncIterableEither<E, B>
  ) => Separated<AsyncIterableEither<E, B>, AsyncIterableEither<E, B>>
  <A>(predicate: Predicate<A>): <E>(
    as: AsyncIterableEither<E, A>
  ) => Separated<AsyncIterableEither<E, A>, AsyncIterableEither<E, A>>
} = <A>(
  predicate: Predicate<A>
): (<E>(as: AsyncIterableEither<E, A>) => Separated<AsyncIterableEither<E, A>, AsyncIterableEither<E, A>>) =>
 ...
 *
 * @example
 * import { partitionWithIndex, fromArray, toTask } from 'fp-ts-iterable/AsyncIterable'
 *
 * async function test() {
 *   const iterable = fromArray([-2, 5, 6, 7])
 *   const { left, right } = partitionWithIndex((index, x: number) => index < 3 && x > 0)(iterable)
 *
 *   assert.deepStrictEqual(await toTask(left), [-2, 7])
 *   assert.deepStrictEqual(await toTask(right), [5, 6])
 * }
 *
 * test()
 *
...
export const partitionWithIndex: {
  ...
  <A>(predicateWithIndex: PredicateWithIndex<number, A>) =>
  <E>(fa: AsyncIterableEither<E, A>): Separated<AsyncIterableEither<E, A>, AsyncIterableEither<E, A>> =>
    separated(
...
export const alt: (
  C: AIChain
) => <E, A>(that: Lazy<AsyncIterableEither<E, A>>) => (fa: AsyncIterableEither<E, A>) => AsyncIterableEither<E, A> = (
  C
) =>
  /*#__PURE__*/
  ET.alt(AI.getMonad(C))
...
export const altW: (
  ...
  that: Lazy<AsyncIterableEither<E2, B>>
) => <E1, A>(fa: AsyncIterableEither<E1, A>) => AsyncIterableEither<E2, A | B> = alt as any
...
export const filterOrElse: {
  ...
  (AIC: AIChain): <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E
  ) => <B extends A>(mb: AsyncIterableEither<E, B>) => AsyncIterableEither<E, B>
  (AIC: AIChain): <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E
  ) => (ma: AsyncIterableEither<E, A>) => AsyncIterableEither<E, A>
} = (AIC) =>
  /*#__PURE__*/
...
export const filterOrElseW: {
  ...
  (AIC: AIChain): <A, E2>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E2
  ) => <E1, B extends A>(mb: AsyncIterableEither<E1, B>) => AsyncIterableEither<E1 | E2, B>
  (AIC: AIChain): <A, E2>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E2
  ) => <E1>(ma: AsyncIterableEither<E1, A>) => AsyncIterableEither<E1 | E2, A>
} = filterOrElse as any
 ...
 * @since 2.7.0
 */
export const getAltAsyncIterableValidation =
  (C: AIChain) =>
  <E>(S: Semigroup<E>): Alt2C<URI, E> => {
    const aalt = ET.altValidation(AI.getMonad(C), S)
    return {
      URI,
      _E: undefined as any,
      map: _map,
      alt: (fa, that) => pipe(fa, aalt(that))
    }
  }
...
export const bracket =
  (M: AIChain) =>
  <E, A, B>(
    acquire: AsyncIterableEither<E, A>,
    use: (a: A) => AsyncIterableEither<E, B>,
    release: (a: A, e: E.Either<E, B>) => AsyncIterableEither<E, void>
  ): AsyncIterableEither<E, B> =>
    bracketW(M)(acquire, use, release)
 ...
 * @since 2.12.0
 */
export const bracketW: (
  C: AIChain
) => <E1, A, E2, B, E3>(
  acquire: AsyncIterableEither<E1, A>,
  use: (a: A) => AsyncIterableEither<E2, B>,
  release: (a: A, e: E.Either<E2, B>) => AsyncIterableEither<E3, void>
) => AsyncIterableEither<E1 | E2 | E3, B> = (C) => {
