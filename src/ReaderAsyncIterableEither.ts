...
import type { Alt3, Alt3C } from 'fp-ts/Alt'
import type { Applicative3, Applicative3C } from 'fp-ts/Applicative'
import { ap as ap_, apFirst as apFirst_, Apply1, Apply3, apS as apS_, apSecond as apSecond_ } from 'fp-ts/Apply'
...
import type { URI as AIURI } from './AsyncIterable'
 ...
 * @since 2.7.0
 */
export function getApplicativeReaderAsyncIterableValidation<E>(
    A: Apply1<AIURI>,
    S: Semigroup<E>
): Applicative3C<URI, E> {
    const aap = ap_(R.Apply, AIE.getApplicativeAsyncIterableValidation(A, S))
    return {
        URI,
        _E: undefined as any,
        map: _map,
 ...
 * @since 2.7.0
 */
export const getAltReaderAsyncIterableValidation =
    (C: AIChain) =>
    <E>(S: Semigroup<E>): Alt3C<URI, E> => {
        const _RAIM = RAI.getMonad(C)
        const aalt = ET.altValidation(_RAIM, S)
        return {
            URI,
            _E: undefined as any,
            map: _map,
 ...
 * @since 2.10.0
 */
export const getAlt = (C: AIChain): Alt3<URI> => {
    const _alt = alt(C)
    return {
        URI,
        map: _map,
...
export const bracketW: (
    C: AIChain
) => <R1, E1, A, R2, E2, B, R3, E3>(
    acquire: ReaderAsyncIterableEither<R1, E1, A>,
    use: (a: A) => ReaderAsyncIterableEither<R2, E2, B>,
    release: (a: A, e: Either<E2, B>) => ReaderAsyncIterableEither<R3, E3, void>
) => ReaderAsyncIterableEither<R1 & R2 & R3, E1 | E2 | E3, B> = C => {
    const _bracketW = AIE.bracketW(C)
    return (acquire, use, release) => r =>
        _bracketW(
            acquire(r),
            a => use(a)(r),
            (a, e) => release(a, e)(r)
...
export const bracket: (
    ...
    acquire: ReaderAsyncIterableEither<R, E, A>,
    use: (a: A) => ReaderAsyncIterableEither<R, E, B>,
    release: (a: A, e: Either<E, B>) => ReaderAsyncIterableEither<R, E, void>
) => ReaderAsyncIterableEither<R, E, B> = bracketW
